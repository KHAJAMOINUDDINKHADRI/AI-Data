...
export function PairwiseComparison() {
  ...
  const criteria: Criterion[] = [
    { id: 'priorityLevel', label: 'Priority Level', description: 'Client priority levels' },
    { id: 'taskFulfillment', label: 'Task Fulfillment', description: 'Completing requested tasks' },
    { id: 'fairness', label: 'Fairness', description: 'Equal distribution' },
    { id: 'workloadBalance', label: 'Workload Balance', description: 'Even work distribution' },
    { id: 'skillMatching', label: 'Skill Matching', description: 'Skills match requirements' },
    { id: 'phasePreference', label: 'Phase Preference', description: 'Preferred phases' }
  ];
  ...
  const [comparisons, setComparisons] = useState<Comparison[]>(generateComparisons());
  ...
  const calculateWeights = () => {
    ...
    const geometricMeans = matrix.map(row => 
      Math.pow(row.reduce((prod, val) => prod * val, 1), 1 / n)
    );
            ...
            {currentIndex < comparisons.length ? (
              <div className="space-y-6">
                ...
                <div className="space-y-4">
                  <div className="text-center">
                    <p className="text-sm font-medium mb-2">Select the more important criterion:</p>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    <Button
                      ...
                      onClick={() => makeComparison(currentComparison.criterion1.id, 3)}
                    >
                      <div className="text-center">
                        <div className="font-medium">{currentComparison.criterion1.label}</div>
                        <div className="text-xs text-gray-500 mt-1">is more important</div>
                      </div>
                    </Button>
                    ...
                    <Button
                      variant="outline"
                      className="h-auto p-4"
                      onClick={() => makeComparison(currentComparison.criterion2.id, 3)}
                    >
                      <div className="text-center">
                        <div className="font-medium">{currentComparison.criterion2.label}</div>
                        <div className="text-xs text-gray-500 mt-1">is more important</div>
                      </div>
                    </Button>
                  </div>
                  ...
                  <div className="text-center">
                    <Button
                      variant="ghost"
                      size="sm"
                      onClick={() => makeComparison(currentComparison.criterion1.id, 1)}
                    >
              ...
              <div className="text-center space-y-4">
                <div className="bg-green-50 border border-green-200 rounded-lg p-6">
                  ...
                  <p className="text-sm text-green-700">
                    You've completed all pairwise comparisons. Click below to calculate the final weights.
                  </p>
                </div>
        ...
        <Card>
          ...
          <CardContent>
            <div className="space-y-2">
              {comparisons.filter(c => c.selected).map((comp, index) => (
                <div key={index} className="flex items-center justify-between text-sm p-2 bg-gray-50 rounded">
                  <div className="flex items-center space-x-2">
                    <span className={comp.selected === comp.criterion1.id ? 'font-medium' : ''}>
                      {comp.criterion1.label}
                    </span>
                    <span className="text-gray-500">vs</span>
                    <span className={comp.selected === comp.criterion2.id ? 'font-medium' : ''}>
